...
最终提交所需的系统问题定义、案例研究、原创贡献、AI 使用声明和截图证据见 [`docs/final-submission-notes.md`](docs/final-submission-notes.md)。
...
- 时间概览以柱高编码年度事件数，内层柱编码当前筛选结果，描边编码窗口参考年份。
...
- 时间详情图表、参战方网络视图、参战方年度热力图、统计面板、可视化事件详情和案例预设。
- 关系网络和参战方年度热力图通过模式切换呈现，避免同时堆叠高密度图表。
- 案例预设可展开动态洞察，比较案例总量、峰值、共现关系和事件类型，并给出“时间—地图—关系—详情”的验证路径。
- 多年度模式同时标明分析范围、窗口参考年份和实际采用的 CShapes 历史边界快照。
- 数据加载失败时提供错误提示和重试入口。
- 筛选、统计和状态联动核心逻辑测试。
...
界面中的“案例预设”提供案例复现入口和紧凑的动态洞察区。选择案例后：

1. 点击“应用窗口”，切换到对应年份范围、定位峰值年份并展开洞察。
2. 点击“聚焦核心参战方”，在相同窗口上追加核心参战方筛选。
3. 按“时间 → 地图 → 关系 → 详情”验证路径复核动态指标和空间分析假设。
...
- `TimelineOverview` 的正式实现、案例洞察与交互优化。
